...
- **Launcher args go to the container command**, not to `podman run` flags. `nix run .# -- foo` runs `foo` inside the container (overriding default CMD `/bin/bash`). Exceptions: `--allow-lan`, `--port`/`-p` are consumed by the launcher.
